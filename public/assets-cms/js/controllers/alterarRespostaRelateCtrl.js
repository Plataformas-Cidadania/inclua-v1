cmsApp.controller('alterarRespostaRelateCtrl', ['$scope', '$http', 'Upload', '$timeout', function($scope, $http, Upload, $timeout){
    ...
    $scope.detalhar = function(id){
        ...
        $http({
            ...
            $scope.dimensoes.forEach(function (item){
                if(item.id_dimensao === $scope.resposta.id_dimensao){
                    $scope.dimensao = item;
                }
            });
            $scope.processandoDetalhar = false;
    ...
    $scope.alterar = function (file){
        ...
        if(file==null){
            ...
            $http.put("api/resposta_relate/"+$scope.id, $scope.resposta).success(function (data){
                //console.log(data);
                $scope.processandoSalvar = false;
            ...
            file.upload = Upload.upload({
                url: 'api/resposta_relate/'+$scope.id,
    ...
    $scope.carregaImagem  = function(img) {
        if(img!=''){
            $scope.imagemBD = 'imagens/respostas_relate/xs-'+img;
