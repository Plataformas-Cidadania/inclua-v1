cmsApp.controller('alterarPerguntaCtrl', ['$scope', '$http', 'Upload', '$timeout', function($scope, $http, Upload, $timeout){
    ...
    $scope.alterar = function (file){
        ...
        if(file==null){
            ...
            $http.put("api/pergunta_relate/"+$scope.id, $scope.pergunta).success(function (data){
                //console.log(data);
                $scope.processandoSalvar = false;
            ...
            file.upload = Upload.upload({
                url: 'api/pergunta_relate/'+$scope.id,
    ...
    $scope.carregaImagem  = function(img) {
        if(img!=''){
            $scope.imagemBD = 'imagens/perguntas_relate/xs-'+img;
