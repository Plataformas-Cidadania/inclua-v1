cmsApp.controller('alterarUrlCtrl', ['$scope', '$http', 'Upload', '$timeout', function($scope, $http, Upload, $timeout){
    ...
    $scope.alterar = function (file, arquivo){
        ...
        if(file==null && arquivo==null){
            ...
            $http.post("cms/alterar-url/"+$scope.id, {
                'url': $scope.url,
            ...
            Upload.upload({
                ...
                $scope.mensagemSalvar =  "Gravado com sucesso!";
                $scope.removerImagem = false;
                $scope.imagemBD = '/imagens/urls/'+response.data;
                $scope.processandoSalvar = false;
    ...
    $scope.carregaImagem  = function(img, arquivo) {
        if(img!=''){
            $scope.imagemBD = 'imagens/urls/xs-'+img;
