cmsApp.controller('alterarFormatoRecursoCtrl', ['$scope', '$http', 'Upload', '$timeout', function($scope, $http, Upload, $timeout){

    $scope.processandoSalvar = false;
    $scope.processandoDetalhar = false;


    //$scope.id_formatoRecurso = 0;
    $scope.dimensoes = [];
    $scope.dimensao = null;
    ...
    $scope.detalhar = function(id){
        ...
        $http({
            ...
            $scope.formatoRecurso = data.data;
            console.log($scope.formatoRecurso);
            $scope.dimensoes.forEach(function (item){
                if(item.id_dimensao === $scope.formatoRecurso.id_dimensao){
                    $scope.dimensao = item;
                }
            });
            $scope.processandoDetalhar = false;
    ...
    $scope.carregaImagem  = function(img) {
        if(img!=''){
            $scope.imagemBD = 'imagens/formatorecurso/xs-'+img;
