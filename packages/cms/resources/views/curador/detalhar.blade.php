...
@section('content')
    {!! Html::script(config('app.url').'assets-cms/js/controllers/alterarCuradorCtrl.js') !!}
    <div ng-controller="alterarCuradorCtrl">
        <div class="box-padrao">
            <h1><a href="javascript:history.back();"><i class="fa fa-arrow-circle-left"></i></a>&nbsp;&nbsp;Curador</h1>
            <?php //print_r($curador);?>
            <div ng-init="carregaImagem('{{$curador->imagem}}', '{{$curador->arquivo}}'); detalhar({{$curador->id_curador}})">
                <span class="texto-obrigatorio">* campos obrigatórios</span><br><br>
                {!! Form::model($curador, ['name' =>'form']) !!}
                ...
                <a href="arquivos/curadores/<% arquivoBD %>" target="_blank" ng-show="arquivoBD"><% arquivoBD %></a>
                <a ng-show="fileArquivo"><% fileArquivo.name %></a>
                {{--<br><br>--}}

                <br><br>
                @include('cms::curador._form')
                <input type="hidden" name="id" ng-model="id" ng-init="id='{{$curador->id_curador}}'"/>
                <div class="row">
                    <div class="col-md-1 col-lg-1 col-xs-3">
                        <br>
                        <button class="btn btn-info" type="button" ng-click="alterar(picFile, fileArquivo)" ng-disabled="form.$invalid && form.curador.$dirty">Salvar</button>
                    </div>
