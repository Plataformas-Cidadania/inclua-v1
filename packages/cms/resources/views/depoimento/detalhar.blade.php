...
@section('content')
    {!! Html::script(config('app.url').'assets-cms/js/controllers/alterarDepoimentoCtrl.js') !!}
    <div ng-controller="alterarDepoimentoCtrl">
        <div class="box-padrao">
            <h1><a href="javascript:history.back();"><i class="fa fa-arrow-circle-left"></i></a>&nbsp;&nbsp;Depoimento</h1>
            <?php //print_r($depoimento);?>
            <div ng-init="carregaImagem('{{$depoimento->imagem}}', '{{$depoimento->arquivo}}'); detalhar({{$depoimento->id_depoimento}})">
                <span class="texto-obrigatorio">* campos obrigatórios</span><br><br>
                {!! Form::model($depoimento, ['name' =>'form']) !!}
                ...
                <a href="arquivos/depoimentos/<% arquivoBD %>" target="_blank" ng-show="arquivoBD"><% arquivoBD %></a>
                <a ng-show="fileArquivo"><% fileArquivo.name %></a>
                {{--<br><br>--}}

                <br><br>
                @include('cms::depoimento._form')
                <input type="hidden" name="id" ng-model="id" ng-init="id='{{$depoimento->id_depoimento}}'"/>
                <div class="row">
                    <div class="col-md-1 col-lg-1 col-xs-3">
                        <br>
                        <button class="btn btn-info" type="button" ng-click="alterar(picFile, fileArquivo)" ng-disabled="form.$invalid && form.depoimento.$dirty">Salvar</button>
                    </div>
