...
@section('content')
    {!! Html::script(config('app.url').'assets-cms/js/controllers/alterarRespostaRelateCtrl.js') !!}
    <div ng-controller="alterarRespostaRelateCtrl">
        <div class="box-padrao">
            <h1><a href="javascript:history.back();"><i class="fa fa-arrow-circle-left"></i></a>&nbsp;&nbsp;Resposta</h1>
            <?php //print_r($resposta);?>
            <div ng-init="carregaImagem('{{$resposta->imagem}}', '{{$resposta->arquivo}}'); detalhar({{$resposta->id_resposta}})">
                <span class="texto-obrigatorio">* campos obrigatórios</span><br><br>
                {!! Form::model($resposta, ['name' =>'form']) !!}
                ...
                <a href="arquivos/respostas/<% arquivoBD %>" target="_blank" ng-show="arquivoBD"><% arquivoBD %></a>
                <a ng-show="fileArquivo"><% fileArquivo.name %></a>
                {{--<br><br>--}}

                <br><br>
                @include('cms::resposta-relate._form')
                <input type="hidden" name="id" ng-model="id" ng-init="id='{{$resposta->id_resposta}}'"/>
                <div class="row">
                    <div class="col-md-1 col-lg-1 col-xs-3">
                        <br>
                        <button class="btn btn-info" type="button" ng-click="alterar(picFile, fileArquivo)" ng-disabled="form.$invalid && form.resposta.$dirty">Salvar</button>
                    </div>
