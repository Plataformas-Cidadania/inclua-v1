...
@section('content')
    {!! Html::script(config('app.url').'assets-cms/js/controllers/urlCtrl.js') !!}
<script>
    $(function () {
    ...
    <div ng-controller="urlCtrl">
        <div class="box-padrao">
            <h1><i class="fa fa-link" aria-hidden="true"></i>&nbsp;Urls</h1>
            <button class="btn btn-primary" ng-click="mostrarForm=!mostrarForm" ng-show="!mostrarForm">Nova Url</button>
            ...
            <div class="col-md-12">
                <div class="box-padrao">
                    ...
                    <table ng-show="totalItens>0" class="table table-striped">
                        <thead>
                        <tr>
                            <th ng-click="ordernarPor('id')" style="urlr:pointer;">
                                Id
                            ...
                            <th>Imagem</th>
                            <th ng-click="ordernarPor('url')" style="urlr:pointer;">
                                Url
                                <i ng-if="ordem=='url' && sentidoOrdem=='asc'" class="fa fa-angle-double-down"></i>
                                <i ng-if="ordem=='url' && sentidoOrdem=='desc'" class="fa fa-angle-double-up"></i>
                            </th>
                        ...
                        <tr ng-repeat="url in urls">
                            <td><% url.id %></td>
                            <td><img ng-show="url.imagem" ng-src="imagens/urls/xs-<% url.imagem %>" width="60"></td>
                            <td><a href="cms/url/<% url.id %>"><% url.titulo %></a></td>
                            <td><a href="cms/apoio/<% url.id %>"><% url.posicao %></a></td>
                            <td class="text-right">
                                <div>
                                    <a><i class="fa fa-arrow-circle-up fa-2x" title="Posição" ng-click="positionUp(url.id);" style="cursor: pointer;" ng-hide="<% $first %>"></i></a>
                                    <a><i class="fa fa-minus-circle fa-2x" title="Posição"   ng-show="<% $first %>" style="color: #CCCCCC; margin-right: 5px;"></i></a>&nbsp;&nbsp;

                                    <a><i class="fa fa-arrow-circle-down fa-2x" title="Posição" ng-click="positionDown(url.id);"  style="cursor: pointer;" ng-hide="<% $last %>"></i></a>
                                    <a><i class="fa fa-minus-circle fa-2x" title="Posição"   ng-show="<% $last %>" style="color: #CCCCCC; margin-right: 5px;"></i></a>

                                    {{--<a href="cms/items/<% url.id %>"><i class="fa fa-sitemap fa-2x" title="Itens"></i></a>&nbsp;&nbsp;--}}
                                    <a href="cms/url/<% url.id %>"><i class="fa fa-edit fa-2x" title="Editar"></i></a>&nbsp;&nbsp;
                                    <a  ng-class="<% url.status %> == 1 ? 'color-success' : 'color-success-inactive'"  style="cursor: pointer;"><i class="fa fa-check-circle fa-2x" aria-hidden="true" ng-click="status(url.id);"></i></a>
                                    <a><i data-toggle="modal" data-target="#modalExcluir" class="fa fa-remove fa-2x" ng-click="perguntaExcluir(url.id, url.titulo, url.imagem)"></i></a>
                                </div>
                            </td>
                        </tr>
                        </tbody>
            ...
            <div class="col-md-12">
                ...
                <div ng-show="totalItens > 0" class="clan-paginacao">
                    <div class="item-paginacao">
                        <uib-pagination total-items="totalItens" ng-model="currentPage" max-size="maxSize" class="pagination-sm" boundary-urls="true" force-ellipses="true" items-per-page="itensPerPage" num-pages="numPages"></uib-pagination>
                    </div>
                    <div class="item-paginacao">
            ...
            <div class="modal-dialog">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-3">
                                <img  ng-src="imagens/urls/xs-<% imagemExcluir %>" width="100">
                            </div>
                            <div class="col-md-9">
                                <p><% tituloExcluir %></p>
                            </div>
